...
int gradient_validation::landau_validation(int argc, char **argv) {
    ...
    GradientCalculator gradient_calculator_opt = GradientCalculator(DIRECTORY_OPTIM_Input.c_str());
    ObjectiveCalculator objective = ObjectiveCalculator(DIRECTORY_OPTIM_Input.c_str());
    OutputControlUpdate outController = OutputControlUpdate(DIRECTORY_OPTIM_Input.c_str());
...
//    control0 = -10.0*control0;
    ...
    in.ReadPlasmaStateForward(forwardParticles,"plasma_state_batch_1_forward_particles_CPU_");
    ...
    in.ReadPlasmaStateBackward(backwardParticles,"plasma_state_batch_1_adjoint_particles_CPU_");
    ...
    forwardPDF = pdf_time;
    assembling_flag = pdf_control.AssemblingMultidimParallel(backwardParticles,1,pdf_time);
    ...
    for(unsigned int i = 0; i< static_cast<unsigned long>(iteration_number); i++) {
        ...
        functional_values[i] = objective.CalculateObjective(forwardPDF,control_temp);
        std::cout << std::to_string(functional_values[i]) << std::endl;
        std::cout << "Stepsize: " << pow(reducing_factor,i) << std::endl;
        difference[i] = (functional_values[i]-functional_value0)-(pow(reducing_factor,i))*product.H2InnerProduct(gradient0,delta_control);
