...
int PdfController::AssemblingMultiDim(
  ...
  pdf_time.clear();
  pdf_time.resize(input_data_->ntimesteps_gp);

  std::vector<std::unordered_map<CoordinatePhaseSpaceTime, double>>
      pdf_time_test(input_data_->ntimesteps_gp);

  std::vector<double> sizeParticles(input_data_->ntimesteps_gp);
  ...
  while (o < input_data_->ntimesteps_gp && return_flag == 0) {
    ...
    for (unsigned int i = 0; i < particles.size(); i++) {
      ...
      if (sqrt(vx * vx + vy * vy + vz * vz) <=
        ...
        binNumberVx = static_cast<int>(std::floor(vx / dv_gp)) +
                      static_cast<int>(std::floor(
                          (vcell_gp / 2.0))); // adding what is left of zero
        binNumberVy = static_cast<int>(std::floor(vy / dv_gp)) +
                      static_cast<int>(std::floor((vcell_gp / 2.0)));
        binNumberVz = static_cast<int>(std::floor(vz / dv_gp)) +
                      static_cast<int>(std::floor((vcell_gp / 2.0)));

        coordinate = CoordinatePhaseSpaceTime(cell_id, binNumberVx, binNumberVy,
                                              binNumberVz, binNumberTime);

        if (pdf_time[o].find(coordinate) == pdf_time[o].end()) {
          pdf_time[o].insert(
              std::pair<CoordinatePhaseSpaceTime, double>(coordinate, 1.0));
        } else {
          pdf_time[o][coordinate]++;
        }
        if (pdf_time_test[o].find(coordinate) == pdf_time_test[o].end()) {
          pdf_time_test[o].insert(
              std::pair<CoordinatePhaseSpaceTime, double>(coordinate, 1.0));
        } else {
        ...
        if (equation_type == 0) {
          ...
          if (too_fast_particles >=
              input_data_->fraction_fast_particles_gp * particles.size()) {
            // logger::Trace("Too many too fast particles, try to increase
            // velocity bound");
            return_flag = 1;
          }
        } else if (equation_type == 1) {
          too_fast_adjoint_particles++;
          // logger::Warning("Particle " + std::to_string(i) + " exceeding
          // velocity bound (adjoint equation)");
        }
      }
    }
    o++;
  ...
  if (equation_type == 0 && too_fast_particles > 0) {
    logger::Info("Particles faster than " +
                 std::to_string(input_data_->vmax_gp) + ": " +
                 std::to_string(too_fast_particles));
  }
  if (equation_type == 1 && too_fast_adjoint_particles > 0) {
    logger::Info("Too fast adjoint particles: " +
                 std::to_string(too_fast_adjoint_particles));
  }
...
int PdfController::AssemblingMultidimParallel(
    std::vector<std::vector<Particle>> &particles_time,
    unsigned int equation_type,
    std::vector<std::unordered_map<CoordinatePhaseSpaceTime, double>>
        &pdf_time) {
  ...
  std::vector<std::unordered_map<CoordinatePhaseSpaceTime, double>>
      pdf_time_test(ntimesteps_gp);

  std::vector<double> sizeParticles(ntimesteps_gp);

  int return_flag = 0;

  // std::cout << "Using " <<  usedThreads << " threads for assembling pdfs" <<
  // std::endl;
...
#pragma omp parallel for
  ...
  std::vector<int>::iterator max_too_fast_particles =
      std::max_element(too_fast_particles.begin(), too_fast_particles.end());
  int number_too_fast_particles = max_too_fast_particles[std::distance(
      too_fast_particles.begin(), max_too_fast_particles)];

  if (equation_type == 0 && number_too_fast_particles > 0) {
    logger::Info("(Maximum) particles faster than " + std::to_string(vmax_gp) +
                 ": " + std::to_string(number_too_fast_particles) +
                 " in iteration " +
                 std::to_string(std::distance(too_fast_particles.begin(),
                                              max_too_fast_particles)));
  }
  if (equation_type == 1 && too_fast_adjoint_particles > 0) {
    logger::Info("Too fast adjoint particles: " +
                 std::to_string(too_fast_adjoint_particles));
...
DoublePDF
PdfController::RelaxatingGaussseidel4D(DoublePDF pdf,
                                       unsigned int number_relaxation_steps) {
  double tolerance_relaxation_gp = input_data_->tolerance_relaxation_gp;
  unsigned int ntimesteps_gp = input_data_->ntimesteps_gp;
  double dt_gp = input_data_->dt_gp;
  double dv_gp = input_data_->dv_gp;
  double vcell_gp = input_data_->vcell_gp;
  ...
  while (n <= number_relaxation_steps && norm > tolerance_relaxation_gp) {
      ...
      for (unsigned int l = 1; l <= vcell_gp - 2; l++) {
        for (unsigned int m = 1; m <= vcell_gp - 2; m++) {
          for (unsigned int n = 1; n <= vcell_gp - 2; n++) {
            relaxated_pdf_New.at(o, l, m, n) =
                1.0 /
                (1.0 + c_s * 2.0 / (dt_gp * dt_gp) +
                 c_s * 6.0 / (dv_gp * dv_gp)) *
                (rhs.at(o, l, m, n) +
                 c_s / (dv_gp * dv_gp) *
                     (relaxated_pdf_New.at(o, l + 1, m, n) +
                      relaxated_pdf_New.at(o, l - 1, m, n) +
                      relaxated_pdf_New.at(o, l, m + 1, n) +
                      relaxated_pdf_New.at(o, l, m - 1, n) +
                      relaxated_pdf_New.at(o, l, m, n + 1) +
                      relaxated_pdf_New.at(o, l, m, n - 1)) +
                 c_s / (dt_gp * dt_gp) *
                     (relaxated_pdf_New.at(o + 1, l, m, n) +
                      relaxated_pdf_New.at(o - 1, l, m, n)));
    ...
    for (unsigned int o = 1; o <= ntimesteps_gp - 2; o++) {
      for (unsigned int l = 1; l <= vcell_gp - 2; l++) {
        for (unsigned int m = 1; m <= vcell_gp - 2; m++) {
          for (unsigned int n = 1; n <= vcell_gp - 2; n++) {
            norm += pow(relaxated_pdf_New.at(o, l, m, n) -
                            relaxated_pdf_Old.at(o, l, m, n),
                        2.0);
          }
        }
      }
    ...
    norm = std::sqrt(norm);
    relaxated_pdf_Old =
        relaxated_pdf_New; //.assign(relaxated_pdf_New.begin(),relaxated_pdf_New.end());
    ++n;
  }
...
double PdfController::CalculateWassersteinMetric(
    std::vector<std::vector<Particle>> dist1,
...
double PdfController::CalculateWassersteinMetricHistogramm(
  ...
  uint vcell_gp = input_data_->vcell_gp;
  double dv_gp = input_data_->dv_gp;
                ...
                for (unsigned int m2 = 0; m2 < vcell_gp; m2++) {
                  for (unsigned int n2 = 0; n2 < vcell_gp; n2++) {
                    ...
                    if (dist1[o].find(c1) != dist1[o].end() ||
                        dist2[o].find(c2) != dist2[o].end()) {

                      std::vector<double> c1_bc =
                          barycenters.find(static_cast<int>(i1))->second;

                      std::vector<double> c2_bc =
                          barycenters.find(static_cast<int>(i2))->second;

                      //-vmax_gp - (-vmax_gp) = 0
                      double distance =
                          (c1_bc[0] - c2_bc[0]) * (c1_bc[0] - c2_bc[0]) +
                      ...
                      wasserstein_value +=
                          distance * (occupation1 + occupation2);
                    }
                  }
                }
              }
