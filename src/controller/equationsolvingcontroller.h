...
public:
  EquationSolvingController(std::shared_ptr<MOTIONS::InputData> &input_data);
  ...
  /**
   * @brief D1_second_order calculates the first derivative with respect to
   * position with a seconr oder accuracy (using the Midpoint scheme)
   *
   * @return
   */
  ...
  /**
   * @brief Laplacian_3D calculates the Laplacion in position in three
   * dimensions
   *
   * @return
   */
