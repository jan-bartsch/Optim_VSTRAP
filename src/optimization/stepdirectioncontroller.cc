...
arma::mat StepdirectionController::GetStepdirection(
  ...
  try {
    direction_update = input_data_->direction_update;
  ...
  if (optimization_iteration == 1) {
    ...
    if (direction_update.compare("negative_gradient") == 0) {
      logger::Info(
          "Updating control using stepdirection update: Negative Gradient");
      return FixedGradientDescent(gradient, optimization_iteration);
    } else if (direction_update.compare("ncg_FR") == 0) {
      logger::Info("Updating control using stepdirection update: NCG using "
                   "Fletcher-Reeves formula");
      return NcgSchemeFR(gradient, gradient_old, stepdirection_old,
                         optimization_iteration);
    } else if (direction_update.compare("ncg_PR") == 0) {
      logger::Info("Updating control using stepdirection update: NCG using "
                   "Fletcher-Reeves formula");
      return NcgSchemePR(gradient, gradient_old, stepdirection_old,
                         optimization_iteration);
    } else if (direction_update.compare("ncg_HZ") == 0) {
      logger::Info("Updating control using stepdirection update: NCG using "
                   "Fletcher-Reeves formula");
      return NcgSchemeHZ(gradient, gradient_old, stepdirection_old,
                         optimization_iteration);
    } else {
...
arma::mat StepdirectionController::NcgSchemeFR(
    arma::mat &gradient, arma::mat &gradient_old, arma::mat &stepdirection_old,
    unsigned int &optimization_iteration) {
...
arma::mat StepdirectionController::NcgSchemePR(
    arma::mat &gradient, arma::mat &gradient_old, arma::mat &stepdirection_old,
    unsigned int &optimization_iteration) {
  arma::mat stepdirection_new;
  double beta = arma::dot(gradient, gradient - gradient_old) /
                arma::dot(gradient_old, gradient_old);
...
arma::mat StepdirectionController::NcgSchemeHZ(
    arma::mat &gradient, arma::mat &gradient_old, arma::mat &stepdirection_old,
    unsigned int &optimization_iteration) {
  arma::mat stepdirection_new;
  arma::mat y = gradient - gradient_old;
  double beta = arma::dot(y - 2.0 * stepdirection_old * arma::dot(y, y) /
                                  arma::dot(stepdirection_old, y),
                          gradient / arma::dot(stepdirection_old, y));
