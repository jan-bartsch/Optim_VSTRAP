    ...
    */
    ...
    for(unsigned int i = 1; i< n+1; i++) {
        ...
        if (current_barycenter[0]> MOTIONS::params::local_control_x_min_gp
            ...
            DoublePDF forwardPDFdouble = DoublePDF();
            DoublePDF backwardPDFdouble = DoublePDF();
                ...
                for(unsigned int l = 0; l<MOTIONS::params::vcell_gp; l++) {
                    for(unsigned int m= 0; m<MOTIONS::params::vcell_gp; m++) {
                        for(unsigned int n = 0; n<MOTIONS::params::vcell_gp; n++) {
                            // derivative v_x
                            if (l == 0) {
                                firstDerivativeForwardPDF_V1_current = (forwardPDFdouble[o][l+1][m][n]-forwardPDFdouble[o][l][m][n])/MOTIONS::params::dv_gp;
                            } else if (l == MOTIONS::params::vcell_gp-1) {
                                firstDerivativeForwardPDF_V1_current = (forwardPDFdouble[o][l][m][n]-forwardPDFdouble[o][l-1][m][n])/MOTIONS::params::dv_gp;
                            } else {
    ...
    for(unsigned long j = 0; j < MOTIONS::params::number_cells_position; j++) {
        if (j>MOTIONS::params::start_control-2 && j<MOTIONS::params::end_control) {
            rhs_Riesz(j-MOTIONS::params::start_control+1,0) = gradient(j,0);
            rhs_Riesz(j-MOTIONS::params::start_control+1,1) = gradient(j,1);
            rhs_Riesz(j-MOTIONS::params::start_control+1,2) = gradient(j,2);
        }
    }
