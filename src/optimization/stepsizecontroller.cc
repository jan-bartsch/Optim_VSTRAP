...
StepsizeController::StepsizeController(std::shared_ptr<MOTIONS::InputData> &input_data) {
...
int StepsizeController::ArmijoLinesearch(arma::mat &gradient, double J0,
    ...
    std::vector<std::vector<Particle>> forwardParticles(input_data_->ntimesteps_gp);
   ...
   */
    ...
    while (Jtemp > J0 + alpha * scalarProduct_Hm * input_data_->armijo_descent_fraction &&
           alpha > input_data_->tolerance_gp && counter <= input_data_->linesearchIteration_max_gp) {
        // while (Jtemp > J0 -
        // alpha*scalarProduct_Stepdirection*armijo_descent_fraction && alpha >
        // tolerance
        //      && counter <= linesearchIteration_max_gp) {
        alpha = pow(input_data_->armijio_base_exp, counter) * input_data_->armijo_iterative_exp * alpha;
        ...
        std::cout << "Armijo: "
                  ...
                  << J0 + alpha * scalarProduct * input_data_->armijo_descent_fraction
                  << std::endl
