...
DataProvider::ReadOptimizationParameters(const char *filename) {
  ...
  TiXmlElement *parameterRoot =
      rootElement->FirstChildElement("globalParameters"); // globalParameters
  ...
  while (parameter) {
    ...
    globalParameters.insert(
        std::pair<std::string, std::string>(name_string, value_string));
...
std::map<std::string, std::string>
DataProvider::ReadSubroutines(const char *filename) {
  ...
  // std::cout << rootElement->Value() << std::endl; //optimizer_Input
  TiXmlElement *parameterRoot =
      rootElement->FirstChildElement("subroutines"); // globalParameters
  // std::cout << parameterRoot->Value() << std::endl;
...
DataProvider::ReadMeshBarycenters(const char *filename) {
  ...
  TiXmlElement *rootElement = InputFile.RootElement();
  // std::cout << rootElement->Value() << std::endl; //optimizer_Input
  TiXmlElement *barycenterRoot =
      rootElement->FirstChildElement("mesh_barycenters"); // globalParameters
  // std::cout << parameterRoot->Value() << std::endl;
  ...
  while (barycenter) {
    ...
    mesh_barycenters.insert(std::pair<int, std::vector<double>>(
        cell_id_int, barycenter_coordinates));
...
arma::mat DataProvider::ConvertBarycentersToArmaMat(
...
std::map<std::string, std::string>
DataProvider::ReadPaths(const char *filename) {
  ...
  // std::cout << rootElement->Value() << std::endl; //optimizer_Input
  TiXmlElement *parameterRoot =
      rootElement->FirstChildElement("paths"); // globalParameters
  // std::cout << parameterRoot->Value() << std::endl;
...
std::map<std::string, std::string> DataProvider::getPaths() const {
  return paths_;
}

void DataProvider::setPaths(const std::map<std::string, std::string> &value) {
  paths_ = value;
}

std::map<std::string, std::string>
DataProvider::getOptimizationParameters() const {
  return optimizationParameters_;
}

void DataProvider::setOptimizationParameters(
...
void DataProvider::setSubroutines(
    const std::map<std::string, std::string> &value) {
  subroutines_ = value;
}
...
void DataProvider::setMesh_barycenters(
    const std::map<int, std::vector<double>> &value) {
