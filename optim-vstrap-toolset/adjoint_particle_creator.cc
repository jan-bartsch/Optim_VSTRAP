...
AdjointParticleCreator::AdjointParticleCreator() : Solver("adjoint_particle_creator") {}
...
void AdjointParticleCreator::ExecuteCPU(const std::string&) {
    for (uint i = 0; i < this->particle_groups_.size(); i++) {
        if ((this->abort_criterium_->GetCurrentItteration() - 1) == this->particle_groups_.at(i).iteration) {
            const uint number_particles(this->particle_groups_.at(i).number_density *
                                        this->particle_groups_.at(i).domain.GetVolume() /
                                        this->particle_groups_.at(i).weight);

            const std::vector<Vector3d> positons =
                    GetRandomNumbers(number_particles, this->particle_groups_.at(i).mu_position,
                                     this->particle_groups_.at(i).sigma_position,0);
            const std::vector<Vector3d> velocities =
                    GetRandomNumbers(number_particles, this->particle_groups_.at(i).mu_velocity,
                                     this->particle_groups_.at(i).sigma_velocity,1);

            auto group = this->data_provider_->GetParticleGroup(this->particle_groups_.at(i).name);

            for (uint k = 0; k < number_particles; k++) {
                group->AddElement();
                group->SetLastStrData(Data::SPECIES, this->particle_groups_.at(i).species);
                group->SetLastDoubleData(Data::WEIGHT, this->particle_groups_.at(i).weight);
                group->SetLastDoubleData(Data::MASS, this->particle_groups_.at(i).mass);
                group->SetLastDoubleData(Data::CHARGE,
                                         constants::kElementalCharge * this->particle_groups_.at(i).charge_number);
...
void AdjointParticleCreator::InitializeCPU(const std::shared_ptr<DataNode>& data) {
    const DataNode node(data->GetNode("executables").GetNode("executable", "name", GetName()));
    const std::string group_name(node.GetNode("particle_group").GetAttribute("name"));
    const Cuboid domain(SetUpSimulationDomain(node));
    const std::vector<DataNode> nodes(node.GetNode("parameraters").GetNodes("set"));

    this->particle_groups_.resize(nodes.size());

    for (uint i = 0; i < nodes.size(); i++) {
        this->particle_groups_.at(i) = GetParticleGroup(group_name, domain, nodes.at(i));
    }
}

void AdjointParticleCreator::InitializeGPU(const std::shared_ptr<DataNode>&) {}

AdjointParticleCreator::Group AdjointParticleCreator::GetParticleGroup(const std::string& name, const Cuboid& domain,
    ...
    group.number_density = std::stod(node.GetNode("particle_values").GetAttribute("number_density"));
    group.iteration = std::stoi(node.GetAttribute("iteration"));
    group.weight = std::stod(node.GetNode("particle_values").GetAttribute("weight"));
    group.charge_number = std::stod(node.GetNode("particle_values").GetAttribute("charge_number"));
...
Cuboid AdjointParticleCreator::SetUpSimulationDomain(const DataNode& node) const {
    Cuboid domain;
...
Vector3d AdjointParticleCreator::GetVector3dValue(const DataNode& node, const std::string& name) const {
    Vector3d value;
...
std::vector<Vector3d> AdjointParticleCreator::GetRandomNumbers(const uint& number_particles, const Vector3d& mu,
                                                               const Vector3d& sigma, int equationType) const {
    ...
    for (uint i = 0; i < 3; i++) {
        ...
        generated_random_numbers = this->random_number_provider_->GetRandomNumbers();

        for (uint k = 0; k < number_particles; k++) {
            random_numbers.at(k)[i] = generated_random_numbers.at(k);
//            while ((domain.max[i]<random_numbers.at(k)[i] || domain.min[i]>random_numbers.at(k)[i])
//                   && equationType == 0) {
//                this->random_number_provider_->GenerateNormalRandomNumbers(mu[i], sigma[i]);
//                generated_random_numbers = this->random_number_provider_->GetRandomNumbers();
//                random_numbers.at(k)[i] = generated_random_numbers.at(k);
