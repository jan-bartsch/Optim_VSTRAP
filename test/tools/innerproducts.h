...
TEST(innerProducts, L2Norm) {
  ...
  arma::mat control(shared_input_data->number_cells_position, 3, arma::fill::randu);

  double inner_product_norm = std::sqrt(pro.L2InnerProduct(control, control));
  double arma_norm = arma::norm(control, "fro") * std::sqrt(shared_input_data->small_discr_volume);
