...
TEST(solver, D1forwardBackward) {
    ...
    EquationSolvingController solver = EquationSolvingController(shared_input_data);
...
TEST(solver, checksecondDerivativeCalculation) {
    ...
    std::map<int, std::vector<double>> barycenters = shared_input_data->barycenters_list;
