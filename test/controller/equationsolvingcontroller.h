...
TEST(solver,checksecondDerivativeCalculation) {
    ...
    std::map<int,std::vector<double>> barycenters = provider.getMeshBarycenters();
    ...
    outContr.WritecontrolXml(1.0/(small_discr_sidelength*small_discr_sidelength)*D1_backward*D1_forward*control);
    outContr.InterpolateControl(provider);
